...
export const selectSystemMenu = async (id: string) =>
  request.get<ResultModel<MenuDetail | null>>({ url: `${MENU_API}/${id}` })

/** 新增菜单。 */
export const createSystemMenu = async (data: MenuSaveRequest) =>
  request.post<ResultModel<string>>({ url: MENU_API, data })

/** 修改菜单。 */
export const updateSystemMenu = async (id: string, data: MenuSaveRequest) => {
  const response = await request.put<ResultModel<boolean>>({ url: `${MENU_API}/${id}`, data })
...
export const deleteSystemMenu = async (id: string) => {
  const response = await request.delete<ResultModel<boolean>>({ url: `${MENU_API}/${id}` })
...
export const changeSystemMenuStatus = async (id: string, status: 0 | 1) => {
  const response = await request.patch<ResultModel<boolean>>({ url: `${MENU_API}/${id}/status/${status}` })
