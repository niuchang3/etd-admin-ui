...
- **表格行高与排版等高规范（Dense Utility 方案 C：Terminal 高密集控制台基准）**：
  - **标准行高统一锁定为 34px**：全站所有业务表格统一使用 `size="small"`，数据行（`tbody > tr > td`）高度固定为 `--du-table-row-height: 34px`，表头行（`thead > tr > th`）固定为 `--du-table-header-height: 32px`，极大提升单屏数据展示效率。
  - **绝对单行化原则**：严禁在一个单元格内垂直堆叠多行文本（例如禁止在一个单元格内上下强挤中文名称与英文 Code）。名称与编码必须拆分为独立列呈现，保证单行高度绝对干净平整。
  - **全表行高绝对齐平**：同一表格内的所有数据行必须保持严格一致的高度，严禁因某一行数据内容较多而忽高忽低。
  - **杜绝行高被撑开的三大纪律**：
    1. **长文本必须单行截断**：文本类列过长时必须统一使用 `EllipsisText` 组件或 CSS 单行省略，严禁文字折行撑高单元格；
    2. **操作按钮统一尺寸**：操作列一律使用 `<a-button type="link" size="small">`，容器使用 `display: flex; align-items: center; justify-content: flex-end;` 垂直居中，尺寸统一为 22px-24px；
    3. **行内控件高度受控**：单元格内的 `StatusTag`、`DictTag`、`a-tag`（如 20px）或 `a-switch`（必须设为 `size="small"`）必须垂直居中且清除多余上下 margin，控件外轮廓总高度不得超过 22px。
