...
- **表格操作列与只读状态展示统一规范（全站强制标准，杜绝留白）**：
  ...
  - **表格二元状态统一 Switch 开关规范（全站强制标准，严禁偏差）**：
    - **Switch 统一标准尺寸（严禁加 size="small"）**：全局所有表格中的业务二元状态控制（如“启用/停用状态”、“安全锁定/正常状态”、“显示/隐藏状态”等）必须一律在单元格内直接使用 Ant Design Vue 默认标准尺寸 `a-switch`（标准高度为 22px，在 34px 单元格内垂直居中），**严禁加 `size="small"`**（避免缩小为微型药丸导致与其他页面尺寸割裂），严禁在表格单元格内使用静态 Tag 标签搭配在操作列冗余堆叠“启用/停用”、“锁定/解锁”等文本按钮；
    - **行内 Switch 样式与状态绑定**：行内 Switch 开关统一绑定行级别独立 loading 状态（如 `:loading="statusChangingId === record.id"`）及权限与安全禁用（如 `:disabled="!canWrite || record.builtIn || isProtected"`），开启文案与关闭文案统一使用 `checked-children` 与 `un-checked-children`（例如“启用/停用”、“锁定/正常”），保持系统经典主题蓝；
    - **操作列职责单一化**：操作列仅保留“编辑、授权、分配、删除”等需要弹窗交互或不可逆破坏性入口，严禁将 Switch 可就地完成的状态切换按钮混入操作列。
...
- **表格开发强制规范（表格行高 34px、Switch 开关、操作按钮防割裂六大刚性铁律）**：
  - **铁律一：标准行高全表绝对锁定 34px，严禁忽高忽低**：全站所有业务表格统一使用 `size="small"`，所有数据行（`tbody > tr > td`）高度刚性锁定为 `--du-table-row-height: 34px`，表头行固定为 `--du-table-header-height: 32px`。同一表格内各行高度必须绝对齐平，严禁因单行内容多少导致行高参差不齐。
  - **铁律二：支持大横滚与固定列，但行高必须刚性齐平（杜绝横滚导致忽高忽低）**：
    - **支持合理横滚**：多列业务表格（列数 ≥ 8 列或宽度超出视口）**完全支持并推荐使用 `:scroll="{ x: ... }"` 与操作列 `fixed: 'right'`**，便于用户浏览宽表；
    - **横滚行高防波动四要素**：
      1. **每列明确定义宽度**：使用横滚时，必须为每列分配明确的 `width`（除允许自适应的描述/备注列外），操作列宽度必须按公式 `按钮数量 × 52px` 充分预留（严禁挤压导致操作按钮被折成两排撑高行高）；
      2. **长文本 100% 截断**：所有可变文本列必须配置 `ellipsis: true` 或使用 `<EllipsisText>`，严禁文字折行造成部分行 34px、部分行 50px 的高低跳跃；
      3. **固定列与主表等高同步**：操作列容器统一为 `.row-actions`（`white-space: nowrap; flex-wrap: nowrap; gap: 2px;`），只读兜底标签统一 `line-height: 24px`，保证固定列（`ant-table-cell-fix-right`）在任何状态下都与主表单元格高度 100% 齐平；
      4. **避免过大估算**：`:scroll="{ x: ... }"` 的数值应紧凑贴合所有列宽之和（例如所有列宽加起来为 1200px 时，设置 `x: 1200` 或 `x: 'max-content'`），杜绝无节制设为 2000px 造成空白过宽。
  - **铁律三：表格容器严禁嵌套冗余边距卡片**：表格外层严禁包裹带有 padding 的 `.table-container` 等冗余容器卡片，统一直接挂在 `.du-panel.table-panel` 下，杜绝双层边框和卡片内边距膨胀。
  - **铁律四：单元格绝对单行化，严禁放置高组件**：严禁在一个单元格内垂直堆叠多行文本（名称与编码必须拆分为独立列）；严禁在表格单元格内使用 `a-avatar` 头像图标、大 margin 容器或徽标；所有标签、Tag、Switch 外边距均已由全局 CSS 消除，确保单行高度绝对干净平整。
  - **铁律五：Switch 开关尺寸全站绝对统一**：表格内 Switch 必须使用默认标准尺寸（高度 22px，在 34px 行高内自然居中），**严禁加 `size="small"`**，颜色统一为系统经典主题蓝，严禁私自修改为绿色等其他颜色。
  - **铁律六：操作列按钮尺寸与列宽预留**：
    - 操作列按钮一律统一使用 `<a-button type="link" size="small">`（高度 24px），放置在 `<div class="row-actions">` 容器内，`white-space: nowrap; flex-wrap: nowrap; gap: 2px;`，严禁换行；
    - 列宽必须根据按钮数量充分预留（每个按钮约 48-52px，2 个按钮至少 120px，3 个按钮至少 160-180px，4 个按钮至少 210-240px），严禁因列宽过窄导致 flex 容器向左溢出侵占前一列（如创建时间）产生文字重叠。
- 表格使用稳定的字符串 `row-key="id"`，操作列保持紧凑；代码、键名、路由和 ID 等数据型文本使用等宽字体。
- **全局排版与字号阶梯规范（方案 C：全盘对齐左侧 11px/12px 微型字）**：
  - **字体族统一**：正文统一使用 `var(--du-font-sans)`，编码、路由、哈希及数据字段统一使用 `var(--du-font-mono)` 或 `.du-mono` 类名。
  - **全站微型字号 Token（全盘对齐左侧侧边栏微型字视觉）**：
    1. `--du-font-size-xs: 11px`：辅助微小字与数据字。用于表头（全大写或等宽）、等宽代码（`code-value`）、创建时间、只读标记（`readonly-label`）、次要标签。
    2. `--du-font-size-sm: 11.5px`（或 `12px`）：全站表格正文与主要数据文字基准。表格数据行首列主实体及普通列文字统一使用 `12px`。
    3. `--du-font-size-base: 12px`：全站右侧控件与正文通用基准（输入框、下拉框、按钮文字全部降为 12px，消除右侧字号膨胀感）。
    4. `--du-font-size-md: 13px`：卡片与弹窗标题。用于 Modal 标题、Panel 面板头部。
    5. `--du-font-size-lg: 14px`：主页面大标题。
  - **标准字重阶梯**：统一使用 400（常规）、500（中等）、600（半粗），全站表格数据行取消 700 大粗体。
  - **表格数据列与首列主实体排版规范（方案 C 基准）**：
    - **表格主实体名称（第一列）统一标准**：全站所有业务表格的第一列主实体字段统一使用 `--du-font-size-sm: 12px` 与 `font-weight: 400`（常规字重），取消首列强加粗与双行堆叠。
    - **整表视觉层级**：表头 11px → 数据列及首列 12px（400 常规平滑） → 辅助编码 11px（400 常规），形成全站与左侧侧边栏 100% 对齐的平滑精致控制台视觉。
- **全站按钮尺寸与场景三级分层规范（严禁尺寸混用与高低不平）**：
  1. **标准控件级（32px 基准，严禁加 small）**：
     - **适用场景**：页面顶部筛选栏/工具栏（查询、重置、新增、批量操作、导出等），以及 Modal 弹窗和 Drawer 抽屉的“确认/取消”底部操作按钮；
     - **严格等高对齐**：必须使用默认尺寸（不加 `size="small"`），高度与 `a-input`、`a-select` 等输入控件 100% 齐平保持在 `--du-control-height: 32px`，严禁在工具栏中随手混入 24px 产生凹凸不平的错落感；
  2. **表格操作列级（24px Link，统一小型化）**：
     - **适用场景**：全站所有业务表格最后一列的操作入口（编辑、删除、添加下级、分配角色等）；
     - **规范形式**：一律统一使用 `<a-button type="link" size="small">`，高度统一为 24px 并垂直居中；内边距由全局 `.row-actions` 统一维护（`padding: 0 4px; gap: 4px; font-size: 12px;`），严禁各业务页面私自手写 padding 覆盖；
  3. **局部微型辅助级（24px Small 紧凑工具）**：
     - **适用场景**：仅用于空间极度受限的局部紧凑辅助工具栏（如弹窗内树控件头部的“全部展开/全部收起/全选/清空”）。
