...
2. 在 `src/apis/<domain>/<module>/type.ts` 定义响应对象、查询参数和保存 DTO，在同目录 `index.ts` 封装接口。
3. 页面放在 `src/views/<domain>/<module>/index.vue`。只有真正跨页面复用的内容才提取到 `src/components`、`src/utils` 或 Store。
4. 页面通常由查询区、紧凑表格、分页、新增/编辑弹窗及必要的确认交互组成；应同时覆盖 loading、空数据、成功反馈和异常恢复。
5. 编辑前需要完整或最新数据时，重新请求详情，不默认相信列表行包含全部字段。
6. 保存、删除、启停成功后刷新受影响的数据；请求中的按钮或控件必须有独立 loading 状态，并在 `finally` 中复位。
...
- 优先使用 Ant Design Vue 现有组件和 `@ant-design/icons-vue`，不引入另一套组件库。
